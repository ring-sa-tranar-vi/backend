...
@Tag(name = "Calls", description = "Endpoints for managing scheduled calls and call completion status.")
public class ScheduledCallController {

    private final ScheduledCallService scheduledCallService;
    private final UserService userService;

    public ScheduledCallController(ScheduledCallService scheduledCallService, UserService userService) {
    ...
    @Operation(summary = "Get user's scheduled calls", description = "Returns all scheduled calls for a user.")
    public ResponseEntity<List<ScheduledCallDto>> getCallsForUser(@PathVariable Long userId) {

        return ResponseEntity.ok(scheduledCallService.getCallsForUser(userId).stream().map(this::toDto).toList());
    }

    @PostMapping("/user/{userId}/reset")
    @Operation(summary = "Reset user's future calls", description = "Cancels future pending calls and recreates weekly calls.")
    @PreAuthorize("#userId == @securityService.currentUserId(authentication.name)")
    ...
    @PreAuthorize("@scheduledCallSecurity.canModify(#id, authentication.name)")
    @Operation(summary = "Mark call as complete", description = "Marks a scheduled call as completed after the call has been attended.")
    ...
    @Operation(summary = "Confirm callback notification received", description = "Confirms that the native app received the callback notification.")
    @PreAuthorize("@scheduledCallSecurity.canModify(#id, authentication.name)")
    ...
    private ScheduledCallDto toDto(ScheduledCall call) {
        return new ScheduledCallDto(call.getId(), call.getUserId(), call.getTrainerId(), call.getTargetTime(), call.getCallBackStatus());
    }
}

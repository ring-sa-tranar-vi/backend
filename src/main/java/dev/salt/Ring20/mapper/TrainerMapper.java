...
public class TrainerMapper {
    ...
    public static RecommendWorkoutResponseDto toRecommendedWorkoutResponse(
            RecommendedWorkoutData data) {
        return new RecommendWorkoutResponseDto(data.workoutId(), data.reasoning());
    }

    public static TrainerResponseDto toResponseDto(
            Trainer trainer, FileStorageService fileStorageService, int validMinutes) {
        String introUrl = getFileUrl(trainer.getIntro(), fileStorageService, validMinutes);

        String imageSelectUrl =
                getFileUrl(trainer.getImageSelect(), fileStorageService, validMinutes);

        String imageCallUrl = getFileUrl(trainer.getImageCall(), fileStorageService, validMinutes);

        String imageStartUrl =
                getFileUrl(trainer.getImageStart(), fileStorageService, validMinutes);

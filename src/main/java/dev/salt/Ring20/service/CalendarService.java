...
public class CalendarService {
    ...
    private List<CalendarEventDto> getWorkouts(Long userId, LocalDateTime start, LocalDateTime end) {
        List<ActivityLog> logs = activityLogRepository.findByUserIdAndStatusAndCompletedAtBetween(
                userId, "COMPLETED", start, end);

        return logs.stream().map(log -> {
            String workoutName = workoutRepository.findById(log.getWorkoutId())
                    .map(Workout::getName)
            ...
            return new CalendarEventDto(
                    ...
                    "WORKOUT",
                    workoutName,
                    "Time: " + (log.getDurationSeconds() != null ? log.getDurationSeconds() / 60 + " min" : "N/A"),
                    log.getCompletedAt(),
                    true);
        }).toList();
    }
    ...
    private List<CalendarEventDto> getEvents(User user, LocalDateTime start, LocalDateTime end) {
        ...
        return user.getAttendingEvents().stream()
                ...
                .map(event -> {
                    String baseDesc = event.getDescription() != null ? event.getDescription() : "";
                    String location = (event.getVenue() != null ? event.getVenue() + ", " : "") + event.getCity();
                    String fullDescription = baseDesc.isEmpty() ? location : baseDesc + " - " + location;
        ...
        for (CallbackPreference pref : preferences) {
            ...
            while (!dateIterator.isAfter(endDate)) {
                if (dateIterator.getDayOfWeek() == targetDay) {
                    ...
                    callEvents.add(new CalendarEventDto(

...
public class UserWorkoutPreferenceService {
    ...
    public Map<String, List<Long>> getPreferences(Long userId) {
        List<Long> dislikedWorkoutIds = preferenceRepository.findByUserIdAndPreferenceType(userId, UserWorkoutPreferenceType.DISLIKED).stream().map(UserWorkoutPreference::getWorkoutId).toList();

        List<Long> favoriteWorkoutIds = preferenceRepository.findByUserIdAndPreferenceType(userId, UserWorkoutPreferenceType.FAVORITE).stream().map(UserWorkoutPreference::getWorkoutId).toList();

        return Map.of("dislikedWorkoutIds", dislikedWorkoutIds, "favoriteWorkoutIds", favoriteWorkoutIds);
    }

    @Transactional
    public UserWorkoutPreference addPreference(Long userId, Long workoutId, UserWorkoutPreferenceType preferenceType) {
        return preferenceRepository.findByUserIdAndWorkoutIdAndPreferenceType(userId, workoutId, preferenceType).orElseGet(() -> {
